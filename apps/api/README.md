...
1. 在服务器上准备 monorepo 根目录 **[`deploy/edge`](../../deploy/edge)**（nginx + certbot），监听 80/443 并接入 Docker 网络 **`edge`**。先启动 edge，再启动 **`nest-admin`**，最后启动 **`react-admin`**，保证业务容器与 `edge-nginx` 在同一 `edge` 网络上。
2. **`react-admin`**：设置 **`BACKEND_UPSTREAM=http://nest-admin:3000`**，由前端反代将浏览器的 **`/api/*`** 转到本后端。
3. **API 子域名**：在 edge 为 API 域名反代到 **`nest-admin:3000`**；若浏览器直接访问 API 子域名，需设置 **`CORS_ORIGINS`**（例如前端 HTTPS 源，多个用英文逗号分隔）。
4. **发版**：本机执行 **[`deploy/local/deploy.sh`](../../deploy/local/deploy.sh)**（`docker build` + `save` + `scp`）。服务器 `.env` 中的 MySQL/JWT 等密钥需事先手工准备；脚本只 upsert **`DOCKER_IMAGE`**。详见 [`CLOUD_DEPLOYMENT.md`](../../CLOUD_DEPLOYMENT.md)。
